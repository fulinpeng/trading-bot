...
const config=require("./config-highesthigh-lowestLow.js");
const {
...
const calculateTradingSignal=() => {
	const curLkine=kLineData[kLineData.length-1];
	...
	if (readyTradingDirection==="up"&&signalUpTerm1&&signalUpTerm2) {
		const highLowStopLoss=calculateHighLow(kLineData, basePeriod*stopLossRatio);
		const highLowStopProfit=calculateHighLow(kLineData, basePeriod*stopProfitRatio);
