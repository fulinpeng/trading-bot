...
const stopLossNum = times.length;
...
// 盈利/止损平仓历史记录
const setCloseOrderHistory = (arr) => {
    if (closeOrderHistory.length < 10) {
        closeOrderHistory.push(arr);
    } else {
        closeOrderHistory.shift();
        closeOrderHistory.push(arr);
    }

    console.log("盈利/止损平仓历史记录 setCloseOrderHistory:", historyEntryPoints);
};
...
const restDatas = (trend, oldOrderCount) => {
    ...
    if (trend === "up") {
        currentPointIndex = 2;
        historyEntryPoints = oldOrderCount ? [..._historyEntryPoints, 2] : [2];
    } else {
        currentPointIndex = 1;
        historyEntryPoints = oldOrderCount ? [..._historyEntryPoints, 1] : [1];
    }
