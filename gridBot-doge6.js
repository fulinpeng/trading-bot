...
const gridPointTrading2 = async () => {
    ...
    if (len === 1) {
        ...
        if (_currentPointIndex === 0) {
            ...
            if (allPoints >= 3 && allPoints % 3 === 0) {
                console.log(`交替穿过${allPoints}次交易点，反手，并且当前index为1，当前交易点的仓位加倍`);
                promises.push(
            ...
            if (allPoints >= 3 && allPoints % 3 === 0) {
                ...
                currentPointIndex = 2;
                historyEntryPoints[allPoints - 1] = 2;
                console.log("到达第四个交易点，重新绘制网格 0/1/2/3交易点改变：", curMaxPrice, gridPoints);
            }
            if (allPoints < 3) {
                if (allPoints % 2 === 0) {
                    const _gridHight = gridPoints[2] - gridPoints[1];
                    if (Math.abs(_gridHight / _currentPrice) < 0.01) {
                        if (allPoints === 2 && Date.now() - firsttradeTime <= (howManyCandleHeight + 1) * 60 * 1000) {
                            ...
                            gridPoints[2] =
                                Math.abs((curMaxPrice - gridPoints[1]) / gridPoints[1]) <= 0.01
                                    ? curMaxPrice * 0.9999999
                                    : gridPoints[1] + gridPoints[1] * 0.01;
            ...
            if (allPoints >= 3 && allPoints % 3 === 0) {
                console.log(`交替穿过${allPoints}次交易点，反手，并且当前index为2，当前交易点的仓位加倍`);
                promises.push(
                    closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex),
                    teadeSell(times[allPoints - 1]),
                );
            ...
            if (allPoints >= 3 && allPoints % 3 === 0) {
                ...
                curMinPrice = gridPoints[1];
                currentPointIndex = 1;
                historyEntryPoints[allPoints - 1] = 1;
                console.log("到达第四个交易点，重新绘制网格 0/1/2/3交易点改变：", curMaxPrice, gridPoints);
            }
            if (allPoints < 3) {
                if (allPoints % 2 === 0) {
                    ...
                    if (Math.abs(_gridHight / _currentPrice) < 0.01) {
                        if (allPoints === 2 && Date.now() - firsttradeTime <= (howManyCandleHeight + 1) * 60 * 1000) {
                            ...
                            // 在2交易完后，设置0/1
                            gridPoints[1] =
                                Math.abs((curMinPrice - gridPoints[2]) / gridPoints[2]) <= 0.01
                                    ? curMinPrice * 0.9999999
                                    : gridPoints[2] - gridPoints[2] * 0.01;
                            gridPoints[0] = gridPoints[1] - (gridPoints[2] - gridPoints[1]) * profitRate; // 不能用 _gridHeight，要实时算
