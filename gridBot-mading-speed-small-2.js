...
const {
    ...
    scaleOverNum,
    scaleHight,
    howTimesToModGridPoints, // 经过几次交易点之内需要修改gridPoints
    howMinutesToGetAaverage, // 修改gridPoints时需要几根k线求极值
} = config["1000pepe"];
...
const getHistoryClosePrices = async () => {
    ...
    candleHeight = calculateCandleHeight(
        getLastFromArr(kLineData, howManyNumForAvarageCandleHight)
    );
...
const setEma = () => {
    maArr = [
        calculateSimpleMovingAverage(
            historyClosePrices.slice(0, historyClosePrices.length - 10),
            maPeriod
        ),
        calculateSimpleMovingAverage(
            historyClosePrices.slice(0, historyClosePrices.length - 5),
            maPeriod
        ),
        calculateSimpleMovingAverage(
            historyClosePrices.slice(0, historyClosePrices.length - 0),
            maPeriod
        ),
    ];
};
...
const placeOrder = async (side, quantity, resetTradingDatas) => {
    ...
    try {
        ...
        if (response && response.data && response.data.orderId) {
            ...
            saveGlobalVariables();
            console.log(
                "placeOrder ~ 下单成功 currentPointIndex tradingDatas:",
                currentPointIndex,
                tradingDatas
            );
...
const recoverHistoryDataByPosition = async (historyDatas, {up, down}) => {
    ...
    // 要恢复仓位，那么 s_ 开头的小仓变量可以直接重置就行，因为即便小仓变成大仓也可以通过他们盈利
    s_money = __s_money;
    s_count = __s_count + 15;
    s_prePrice = __s_prePrice;
    ...
    if (
        up &&
        __currentPointIndex === 3 &&
        currentPrice > gridPoints[2] &&
        currentPrice < gridPoints[3]
    ) {
        currentPointIndex = 2;
        prePointIndex = 2;
    ...
    if (
        down &&
        __currentPointIndex === 0 &&
        currentPrice > gridPoints[0] &&
        currentPrice < gridPoints[1]
    ) {
        currentPointIndex = 1;
        prePointIndex = 1;
    ...
    if (isResting) {
        ...
        if (currentPrice <= gridPoints[0]) {
            console.log(
                `初始化时，上一轮回处于休息状态，价格超出网格区间，重置仓位，当前价格小于gridPoints[0]`
            );
        ...
        if (currentPrice >= gridPoints[3]) {
            console.log(
                `初始化时，上一轮回处于休息状态，价格超出网格区间，重置仓位，当前价格大于gridPoints[3]`
            );
...
const setHistoryEntryPoints = (point) => {
    ...
    console.log(
        `${isResting ? "休息中，" : ""}进入交易点的历史记录 historyEntryPoints:`,
        historyEntryPoints
    );
};
        ...
        if (tradingDatas[pointIndex].up) {
            ...
            await closeOrder("SELL", quantity, () => {
                ...
                if (showProfit) {
                    ...
                    testMoney += dis;
                    console.log(
                        "平多 closePointOrders ~ currentPrice testMoney:",
                        currentPrice,
                        testMoney
                    );
                }
        ...
        if (tradingDatas[pointIndex].down) {
            ...
            await closeOrder("BUY", quantity, () => {
                ...
                if (showProfit) {
                    ...
                    testMoney += dis;
                    console.log(
                        "平空 closePointOrders ~ currentPrice testMoney:",
                        currentPrice,
                        testMoney
                    );
                }
    ...
    if (up) {
        ...
        const upPromise = closeOrder("SELL", up.quantity, () => {
            if (showProfit) {
                ...
                console.log(
                    "平多 closeAllOrders ~ currentPrice testMoney:",
                    currentPrice,
                    testMoney
                );
            }
    ...
    if (down) {
        ...
        const downPromise = closeOrder("BUY", down.quantity, () => {
            if (showProfit) {
                ...
                console.log(
                    "平空 closeAllOrders ~ currentPrice testMoney:",
                    currentPrice,
                    testMoney
                );
            }
    ...
    if (len === 1) {
        if (isResting) {
            ...
            if (_currentPointIndex === 0) {
                console.log(
                    `休息中到达交易点0，交替穿过${allPoints}次交易点，退出休息模式继续开单`
                );
                ...
                console.log(
                    `休息中到达交易点3，交替穿过${allPoints}次交易点，退出休息模式继续开单`
                );
        ...
        if (_currentPointIndex === 0) {
            console.log(
                `交替穿过${allPoints}次交易点，是 1~0，重置仓位（盈利）！！！，并当前继续开空`
            );
            await closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex);
            ...
            console.log(
                `交替穿过${allPoints}次交易点，是 2~3，重置仓位（盈利）！！！，并当前继续开多`
            );
            await closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex);
            ...
            if (nextBig && !isResting && allPoints === overNumberToRest) {
                console.log(
                    "仓位过大，暂存该交易：currentPrice, gridPoints",
                    currentPrice,
                    gridPoints
                );
                await closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex);
            ...
            // >>>>
            if (allPoints <= howTimesToModGridPoints) {
                const max = Math.max(
                    ...getLastFromArr(kLineData, howMinutesToGetAaverage).map((v) => v.high)
                );
                if (gridPoints[2] < max) {
            ...
            if (nextBig && !isResting && allPoints === overNumberToRest) {
                console.log(
                    "仓位过大，暂存该交易，重新开始：currentPrice, gridPoints",
                    currentPrice,
                    gridPoints
                );
                await closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex);
            ...
            if (allPoints <= howTimesToModGridPoints) {
                const min = Math.min(
                    ...getLastFromArr(kLineData, howMinutesToGetAaverage).map((v) => v.low)
                );
                if (gridPoints[1] > min) {
...
const beforStartRunGrid = async (profit) => {
    ...
    if (orderInfo && ((!orderInfo.up && orderInfo.down) || (orderInfo.up && !orderInfo.down))) {
        ...
        console.log(
            "仓位错误: tradingDatas currentPointIndex hasOrder",
            tradingDatas,
            currentPointIndex,
            hasOrder
        );
    ...
    if (trend === "up") {
        dis =
            quantity * (_currentPrice - orderPrice) -
            quantity * (orderPrice + _currentPrice) * 0.0007;
        _money.push(dis);
    }
    if (trend === "down") {
        dis =
            quantity * (orderPrice - _currentPrice) -
            quantity * (orderPrice + _currentPrice) * 0.0007;
        _money.push(dis);
    }
    ...
    if (sum >= profit) {
        ...
        console.log(
            `交替穿过${historyEntryPoints.length}次交易点，是 小仓win，重置仓位（盈利）！！！`
        );
...
const startRunGrid = async (_prePrice, _currentPrice) => {
    ...
    for (let index = 0; index < gridPoints.length; index++) {
        ...
        if (
            (_prePrice <= point && point <= _currentPrice) ||
            (_prePrice >= point && point >= _currentPrice)
        ) {
...
const createLogs = () => {
    ...
    logStream = fs.createWriteStream(
        `${logsFolder}/${isTest ? "test" : "prod"}-mading-${SYMBOL}-${getDate()}.log`,
        {
            flags: "a",
        }
    );
...
const test = async () => {
    ...
    console.log(
        "🚀 ~ file: gridBot6-1.js:886 ~ startTrading ~ allPositionDetail:",
        allPositionDetail
    );
...
function saveGlobalVariables() {
    setTimeout(() => {
        ...
        if (currentPrice !== 0 && prePrice !== 0) {
            ...
            fs.writeFileSync(
                `data/${isTest ? "test" : "prod"}-mading-${SYMBOL}.js`,
                `module.exports = ${data}`,
                {
                    flag: "w",
                }
