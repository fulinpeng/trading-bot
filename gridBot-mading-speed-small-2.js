    ...
    if (len === 1) {
        ...
        if (_currentPointIndex === 0) {
            ...
            if (allPoints <= howTimesToModGridPoints) {
                const max = Math.max(...getLastFromArr(kLineData, howMinutesToGetAaverage).map((v) => v.high));
                if (gridPoints[2] < max) {
            ...
            // >>>>
            if (allPoints <= howTimesToModGridPoints) {
                const min = Math.min(...getLastFromArr(kLineData, howMinutesToGetAaverage).map((v) => v.low));
                if (gridPoints[1] > min) {
