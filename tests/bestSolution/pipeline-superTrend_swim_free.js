...
const paramRangesObj = {
    atrPeriod: { min: 9, max: 15, step: 1 },
    multiplier: { min: 10, max: 30, step: 0.5 },
    firstProtectProfitRate: { min: 0, max: 0.5, step: 0.5 },
    arriveStopProfitCount: { min: 1, max: 3, step: 1 },
    swimingFreePeriod: { min: 40, max: 60, step: 10 },
    sslPeriod: { min: 200, max: 200, step: 20 },
    sslRateUp: { min: -0.001, max: 0.001, step: 0.00002 },
    sslRateDown: { min: -0.001, max: 0.001, step: 0.00002 },
};
...
function areSolutionsEqual(solutionA, solutionB) {
    return (
        solutionA.atrPeriod === solutionB.atrPeriod &&
        solutionA.multiplier === solutionB.multiplier &&
        solutionA.firstProtectProfitRate === solutionB.firstProtectProfitRate &&
        solutionA.arriveStopProfitCount === solutionB.arriveStopProfitCount &&
        solutionA.swimingFreePeriod === solutionB.swimingFreePeriod &&
        solutionA.sslPeriod === solutionB.sslPeriod &&
        solutionA.sslRateUp === solutionB.sslRateUp &&
        solutionA.sslRateDown === solutionB.sslRateDown
    );
}
    ...
    for (let i = 0; i < batch.length; i++) {
        childProcesses.push(
            new Promise((resolve, reject) => {
                const child = fork(path.join(__dirname, "./childProcess/child-superTrend_swim_free-tail.js")); // 创建子进程
