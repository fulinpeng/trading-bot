 ...
 *            也可以是 0.08 0.1 等
 * 3. 伸缩 gridHeight 提高胜率，减少了交易点徘徊数量
 *    但是，，，收益变低，风险也未必降低，不太可取
 *
 */
...
let max = 0;
const start = () => {
    // let index = kLineData.findIndex((v) => v.openTime === "2024-04-14_00-00-00");
    ...
    for (let idx = 100; idx < kLineData.length; idx++) {
        ...
        if (min) {
            if (min > curkLine.low) min = curkLine.low;
        } else {
            min = curkLine.low;
        }
        if (max) {
            if (max < curkLine.high) max = curkLine.high;
        } else {
            max = curkLine.high;
        }
...
const gridPointTrading2 = () => {
    ...
    if (historyEntryPointsLlen == 3 || historyEntryPointsLlen == 5) {
        if (trend === "up") {
            if (min < gridPoints[1]) gridPoints[1] = min;
        }
        if (trend === "down") {
            if (max > gridPoints[2]) gridPoints[2] = max;
        }
    }
};
