...
const setEveryIndex = (curKLines) => {
    ...
    superTrendArr.push(
        calculateIndicators(curKLines, demaShortPeriod, demaLongPeriod, atrPeriod, multiplier)
    );
};
                    ...
                    if (firstStopProfitRate) {
                        ...
                        if (close > firstProfitPrice) {
                            ...
                            if (close < open) {
                                ...
                                gridPoints[0] =
                                    orderPrice +
                                    Math.abs(orderPrice - firstProfitPrice) *
...
const calculateTradingSignal = (kLines) => {
    ...
    if (readyTradingDirection === "up" && close > Math.max(demaShort, demaLong)) {
        // &&signalUpTerm1
    ...
    if (readyTradingDirection === "down" && close < Math.min(demaShort, demaLong)) {
        // &&signalDownTerm1
...
run({
    ...
    double: 1,
    maxLossCount: 20,
    // targetTime: "2023-10-01_00-00-00",
});
module.exports = {
