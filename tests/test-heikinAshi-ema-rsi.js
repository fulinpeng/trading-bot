...
let { kLineData: heikinAshiKLineData } = require(`./source/heikinAshi-${symbol}-1m.js`);

const DefaultAvailableMoney = 10;
let maxAvailableMoney = 0;
let numForAverage = 0;
let _kLineData = [...kLineData];
// let heikinAshiKLineData=_heikinAshiKLineData=convertToHeikinAshiArr(kLineData)
let _heikinAshiKLineData = [...heikinAshiKLineData];
let curKLines = [];
let double = 0;
let lossCount = 0;
let maxLossCount = 2;
...
const start = (params) => {
    ...
    if (targetTime) {
        ...
        _heikinAshiKLineData = [...heikinAshiKLineData].slice(start - 500);
    }

    stochRSICalc = createStochRSICalculator(
        stochRsiPeriods.smoothK,
        stochRsiPeriods.smoothD,
        stochRsiPeriods.lengthRSI,
        stochRsiPeriods.lengthStoch
    );

    const preKLines = _heikinAshiKLineData.slice(0, 500);
    initEveryIndex(preKLines);
    for (let idx = 500; idx < _kLineData.length; idx++) {
        curKLines = _kLineData.slice(idx - 500, idx);
        const heikinAshiCurKLines = _heikinAshiKLineData.slice(idx - 210, idx);

        candleHeight = calculateCandleHeight(_kLineData.slice(idx - numForAverage, idx));

        // 设置各种指标
        setEveryIndex(heikinAshiCurKLines);
...
const calculateTradingSignal = (kLines) => {
    ...
    // 计算ATR
    const atr = calculateATR(curKLines.slice(-20), atrPeriod).atr;
...
run({
    ...
    firstStopProfitRate: 3.5,
    firstProtectProfitRate: 0.9,
    ...
    double: 1,
    maxLossCount: 20,
    targetTime: "2025-02-24_00-00-00",
});
module.exports = {
