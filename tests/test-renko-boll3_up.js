 ...
 */
function findIntersection(line1, line2, startIndex, maxLookahead=100) {
...
function detectTriangleBreak( kLines, maxLookahead = 50) {
    ...
    if (isUpOpen) {
        // 顶底差在逐渐减小
        if (close >= B2upper && Math.abs(low4 - high3) > Math.abs(low5 - high4) * 1.5) {
            highPoint = [[highIndex3, high3], [highIndex4, high4]];
            lowPoint = [[lowIndex4, low4], [lowIndex5, low5]];
            startIndex = highIndex3 < lowIndex4 ? highIndex3 : lowIndex4
        }
        ...
        // 顶底差在逐渐减小
        if (close <= B2lower && Math.abs(high4 - low3) > Math.abs(high5 - low4) * 1.5) {
            highPoint = [[highIndex4, high4], [highIndex5, high5]];
            lowPoint = [[lowIndex3, low3], [lowIndex4, low4]];
            startIndex = highIndex4 < lowIndex3 ? highIndex4 : lowIndex3
        }
...
function detectPingxingBreak(kLines) {
    ...
    // 这里也有差别
    // if (pingxingIndex && isUpOpen && idx - pingxingIndex > 10) return null;
    let [highIndex1, highIndex2, highIndex3, highIndex4, highIndex5] = getLastFromArr(currentIndexHigh, 5);
    let [lowIndex1, lowIndex2, lowIndex3, lowIndex4, lowIndex5] = getLastFromArr(currentIndexLow, 5);
    let [kLine1, kLine2, kLine3, kLine4, kLine5] = getLastFromArr(kLines, 5);
    let { openTime, closeTime, high, low, close } = kLine5;
...
const calculateTradingSignal = (kLines) => {
    ...
    // let max = isDetectPingxingBreak ? (high5 + low5) / 2 : B2upper;
    // let min = isDetectPingxingBreak ? (high5 + low5) / 2 : B2lower;
    let max = B2upper;
    let min = B2lower;
    ...
    if (signalUpTerm0 && signalUpTerm1) {
        ...
        return {
            trend: "up",
            stopLoss: min, // 止损
            stopProfit: close + Math.abs(close - min) * howManyCandle, // 止盈
        };
    }
    ...
    if (signalDownTerm0 && signalDownTerm1) {
        ...
        return {
            trend: "down",
            stopLoss: max, // 止损
            stopProfit: close - Math.abs(close - max) * howManyCandle, // 止盈
        };
    }
    return {
        trend: "hold",
...
run({
    ...
    B2mult: 1.5, // boll倍数
    howManyCandle: 3, // 止盈
    firstStopProfitRate: 1.5,//2, // 多少个 brickSize （首次止盈，只用一次后失效）
    firstProtectProfitRate: 0.1, // 0.9, // firstStopProfitRate > 0 时生效，达到首次止盈保留多少利润
    // 该策略不适合用firstStopLossRate
