...
const judgeTradingDirection=(kLines) => {
	...
	const upTerm1=halfTrend3.trend===1&&halfTrend5.trend===0;
	const upTerm2=rsi5>rsiPoints[1];
	if (upTerm1&&upTerm2) {
		readyTradingDirection="up";
		return
	}

	const downTerm1=halfTrend3.trend===0&&halfTrend5.trend===1;
	const downTerm2=rsi5<rsiPoints[0];
...
const judgeBreakTradingDirection=(kLines) => {
	...
	const upTerm1=halfTrend5.trend==0
	const upTerm2=rsi5>30;
	if (readyTradingDirection==="up"&&!(upTerm1&&upTerm2)) {
		readyTradingDirection="hold"
		return
	}

	const downTerm1=halfTrend5.trend==1
	const downTerm2=rsi5<70;
...
const calculateTradingSignal=(kLines) => {
	...
	const signalUpTerm2=close>halfTrend5.ht
	...
	const signalDownTerm2=close<halfTrend5.ht
...
// 用了trend 为0/1判断多空 + rsi > rsiPoints[1]，做的是突破单