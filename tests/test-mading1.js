...
const initNum = 11; // parseInt(times.length / 3);
const shortPointLen = 15; // shortPointLen 个对冲次数盈利说明趋势明显，不是盘整
...
const gridPointTrading2 = () => {
    ...
    if (_currentPointIndex === 0) {
        ...
        if (isScaleProfitRate && (curProfitRate !== profitRate || curDiff !== diff)) {
            curProfitRate = profitRate;
            curAtrRate = atrRate;
            console.log(curkLine.openTime, "🚀 ++++ file: curProfitRate,curAtrRate:", curProfitRate, curAtrRate);
        ...
        // 缩放止盈比例
        if (isScaleProfitRate && historyEntryPointsLlen >= scaleProfitRateNum) {
            curProfitRate = 2;
            curAtrRate = atrRate * 2;
            console.log(curkLine.openTime, "🚀 ---- file: curProfitRate,curAtrRate:", curProfitRate, curAtrRate);
            times = getSequenceArr(curDiff, 100);
