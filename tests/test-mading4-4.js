 ...
 *            也可以是 0.08 0.1 等
 * 3. 在 test-mading4-3.js 基础上根据仓位增大profit，方法：getProfit
 *  不稳定，不太行，
 *  可能是不好覆盖损失
 */

const { getLastFromArr, getSequenceArr, getRatioArr } = require("../utils/functions");
...
// let times = getSequenceArr(diff, 100);
let times = getRatioArr(1.07, 100);
let modelType = 1;
const model1 = {
    timeDis: 5,
    profit: 0.1,
...
const getProfit = (profit) => {
    let ratio = (availableMoney * times[historyEntryPoints.length - 1]) / (availableMoney * times[0]);
    return profit * ratio;
};
const start = () => {
    let index = kLineData.findIndex((v) => v.openTime === "2024-06-02_04-40-00");
            ...
            if (hasOrder) {
                if (modelType === 1) {
                    if (s_count % model1.timeDis === 0) {
                        s_prePrice = curkLine.close;
                        needContinue = beforStartRunGrid(curkLine, getProfit(model1.profit));
                ...
                if (modelType === 2) {
                    if (Math.abs(s_prePrice - curkLine.close) / s_prePrice >= model2.priceDis) {
                        s_prePrice = curkLine.close;
                        needContinue = beforStartRunGrid(curkLine, getProfit(model2.profit));
    ...
    for (let index = 0; index < gridPoints.length; index++) {
        ...
        if (low <= gridPoints[1] && high >= gridPoints[2]) {
            ...
            if (curkLine.close > curkLine.open) {
                ...
                // gridPoints[2] += (gridPoints[2] - gridPoints[1])/2;
            } else {
                // down
                _currentPointIndex = 1;
                beforeGridPointTrading2(_currentPointIndex);
                // gridPoints[1] -= (gridPoints[2] - gridPoints[1])/2;
            }
