...
const protectProfit = true; // true false; // 更保守的话开启利润保护
...
const gridPointTrading2 = () => {
    ...
    if (_currentPointIndex === 0) {
        ...
        if (
            canStop &&
            !isResting &&
            (historyEntryPointsLlen === overNumberToRest || historyEntryPointsLlen - overNumberToRest === 2)
        ) {
            if (historyEntryPointsLlen === overNumberToRest) {
                closeOrderHistory.push(_historyEntryPoints.map((v) => 0));
                // overNumberHistory.push(historyEntryPointsLlen);
                setLinesClose();
            }
            // if (historyEntryPointsLlen - overNumberToRest === 2) {
            //     closeOrderHistory.push(_historyEntryPoints.map((v) => 0));
            //     setLinesClose();
            // }
            isResting = true;
