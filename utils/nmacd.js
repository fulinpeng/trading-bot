...
function calculateNormalizedMACD(prices, params = { sma: 12, lma: 21, tsp: 9, np: 50, type: 1 }) {
    ...
    // 步骤 2：计算 ratio 和原始 MAC 值
    const ratio = shortMA.map((sh, i) => Math.min(sh, longMA[i]) / Math.max(sh, longMA[i]));
    const mac = ratio.map((r, i) => (shortMA[i] > longMA[i] ? 2 - r : r) - 1);

    // 步骤 3：归一化 MAC 值
    const lowestMac = lowest(mac, normalizePeriod);
    const highestMac = highest(mac, normalizePeriod);
    const macNorm = mac.map((m, i) => {
        if (normalizePeriod < 2) return m;
        return ((m - lowestMac[i]) / (highestMac[i] - lowestMac[i] + 0.000001)) * 2 - 1;
    });

    // 步骤 4：触发线（Trigger）的计算
    const trigger = wma(macNorm, triggerPeriod);
    ...
    // 返回当前 K 线的指标值
    // macNorm 是归一化的 MACD 值，表示当前市场强弱的核心指标。
    // trigger 是基于 macNorm 的触发线，用于发出买入或卖出信号。
    // hist 是 macNorm 和 trigger 的差值，通过柱状图的形式直观地显示市场的趋势变化。
    return {
