...
function calculateSHK(klineData, params) {
	const {stcLength, fastLength, slowLength, smoothingFactor}={...defaultParams, ...params};
	...
	function calcSTC(kLines, stcLength, fastLength, slowLength) {
		...
		// 计算EMA差值数组
		const emaDiff=calcEMA_diff(kLines.map(kline => kline.close), fastLength, slowLength);
		...
		// 计算相对强弱的百分比变化
		const k=rangeEmaDiff>0? (emaDiff[emaDiff.length-1]-minEmaDiff)/rangeEmaDiff*100:preSmoothedK;

		// 使用指数移动平均对K值进行平滑, 最新的 preSmoothedK
		const currentSmoothedK=!preSmoothedK? k:preSmoothedK+smoothingFactor*(k-preSmoothedK)
		...
		// 计算D值
		const d=rangeSmoothedK>0? (currentSmoothedK-minSmoothedK)/rangeSmoothedK*100:smoothedD;
...
function calcEMA_diff(closePrices, fastLength, slowLength) {
	...
	return fastEMA.map((fast, index) => (fast!==null&&slowEMA[index]!==null? fast-slowEMA[index]:null));
}
...
function lowest(arr, length) {
	...
	return arr.slice(-length).reduce((min, val) => (val!==null&&val<min? val:min), Infinity);
}
...
function highest(arr, length) {
	...
	return arr.slice(-length).reduce((max, val) => (val!==null&&val>max? val:max), -Infinity);
}
