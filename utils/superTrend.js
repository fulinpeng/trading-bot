...
function calculateSuperTrend(klineData, atrPeriod, multiplier) {
	...
	for (let i=atrPeriod;i<klineData.length;i++) {
		...
		trend[i]=i>atrPeriod? (trend[i-1]===-1&&klineData[i].close>dn[i-1]? 1:(trend[i-1]===1&&klineData[i].close<up[i-1]? -1:trend[i-1])):1;
	}
	return {
...
function calculateIndicators(klineData, demaShortPeriod=144, demaLongPeriod=169, atrPeriod=10, multiplier=3, demaSource='close') {
	let closePrices=demaSource==='hl2'? klineData.map(k => (k.high+k.low)/2):klineData.map(k => (k.close));
