...
function calculateHalfTrend(kLines, amplitude, channelDeviation, preData) {
	...
	const {trend: preTrend, nextTrend: preNextTrend, up: preUp, down: preDown, maxLowPrice: preMaxLowPrice, minHighPrice: preMinHighPrice, }=preData||{}
	...
	const highma=sma(kLines.slice(-amplitude).map((item) => item.high), amplitude);
	const lowma=sma(kLines.slice(-amplitude).map((item) => item.low), amplitude);
