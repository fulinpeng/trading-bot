...
const addSize = (n) => n >= max ? DefaultAvailableMoney : DefaultAvailableMoney; // 每次加仓10u ★★★★ 这里也挺重要
    ...
    try {
        ...
        if (isLongOpen && longPositions.length > 0) {
            if (price > maxHigh * (1 - gridRatio)) {
                ...
                availableMoney += size;
                maxHigh = price; // price
                console.log("🚀 ~ executeTrade ~ 多仓加仓, price, size, maxHigh:", price, size, maxHigh);
            }
            else {
               if (price < B2lower && kLine5.close < kLine5.open && price < longPositions[longPositions.length - 1].price) maxHigh = price;
           }
        }
        ...
        if (isShortOpen && shortPositions.length > 0) {
            if (price < minLow * (1 + gridRatio)) {
                ...
                availableMoney += size;
                minLow = price; // price
                console.log("🚀 ~ executeTrade ~ 空仓加仓, price, size, minLow:", price, size, minLow);
            }
            else {
               if (price > B2upper && kLine5.close > kLine5.open && price > shortPositions[shortPositions.length - 1].price) minLow = price;
           }
        }

        // 初始化时开多仓
        if (isLongOpen && longPositions.length === 0 && price > B2upper) {
            ...
            maxHigh = price;
            console.log(
                "🚀 ~ executeTrade ~ 开多仓 price, initialSize, availableMoney, maxHigh:",
                price,
                initialSize,
                availableMoney,
                maxHigh
        ...
        // 初始化时开空仓
        if (isShortOpen && shortPositions.length === 0 && price < B2lower) {
            let quantity = getQuantity(initialSize, price);
            await teadeSell(quantity);
            // 开空仓
            shortPositions.push({
            ...
            minLow = price;
            console.log(
                "🚀 ~ executeTrade ~ 开空仓 price, initialSize, availableMoney, minLow:",
                price,
                initialSize,
                availableMoney,
                minLow
