...
let tradingSwitch = TradingStatus.Play;
...
const gridPointTrading2 = async () => {
    ...
    const len = pointIndexHistory.length;
    const promises = [];
    if (len === 1 && tradingSwitch === TradingStatus.Play) {
        console.log(`在第1个交易点`, tradingDatas);
    } else if (len === 3 && tradingSwitch === TradingStatus.Play) {
        console.log(
        ...
        if (_currentPointIndex === 0) {
            ...
            // 发送邮件
            sendMail({
                subject: `${B_SYMBOL}不可能到了第3个交易点，并且此时价格未触发止盈，请手动处理`,
                text:
                    `交替穿过${allPoints}次交易点` +
                    JSON.stringify({
                        gridPoints,
            ...
            console.log(
                `到了第3个交易点，可能出现亏损，historyEntryPoints, tradingDataArr`,
                _historyEntryPoints,
                tradingDataArr,
            );
        }
    } else if (len === 2 || tradingSwitch === TradingStatus.Pause) {
        console.log(
        ...
        // 2 个交易点之间交替
        if (_currentPointIndex === 0) {
            if (tradingSwitch === TradingStatus.Pause) {
                tradingSwitch = TradingStatus.Play;
                console.log(`交替穿过00${allPoints}次交易点，重置仓位（未盈利）当前交易处于停止状态，重新开启！！`);
                // 重置网格
                const _gridHight = gridPoints[2] - gridPoints[1];
                gridPoints[1] = _currentPrice * 0.9999999;
                gridPoints[2] = gridPoints[1] + _gridHight;
                gridPoints[3] = gridPoints[2] + _gridHight * stopLossRate;
                gridPoints[0] = gridPoints[1] - _gridHight * stopLossRate;

                currentPointIndex = 1;
                historyEntryPoints = new Array(stopCount).fill(-1);
                await teadeSell(times[stopCount], true);
            } else {
            ...
            if (tradingSwitch === TradingStatus.Pause) {
                ...
                // 重置网格
                const _gridHight = gridPoints[2] - gridPoints[1];
                gridPoints[2] = _currentPrice * 0.9999999;
                gridPoints[1] = gridPoints[2] - _gridHight;
                gridPoints[3] = gridPoints[2] + _gridHight * stopLossRate;
                gridPoints[0] = gridPoints[1] - _gridHight * stopLossRate;

                currentPointIndex = 2;
                historyEntryPoints = new Array(stopCount).fill(-2);

                await teadeBuy(times[stopCount], true);
            } else {
        ...
        if (tradingSwitch === TradingStatus.Play) {
            if (_currentPointIndex === 1) {
                if (allPoints === stopCount) {
                    console.log(`交替穿过${allPoints}次交易点，平仓后停止交易`);
                    promises.push(closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex));
                    await Promise.all(promises);
                    tradingSwitch = TradingStatus.Pause;
                } else {
                    ...
                    await Promise.all(promises);
                    const _gridHight = gridPoints[2] - gridPoints[1];
                    if (allPoints <= stopCount && Math.abs(_gridHight / _currentPrice) < 0.01) {
            ...
            if (allPoints === stopCount) {
                const _gridHight = gridPoints[2] - gridPoints[1];
                gridPoints[3] = gridPoints[2] + (_gridHight * (stopLossRate + 1)) / 2;
                gridPoints[0] = gridPoints[1] - (_gridHight * (stopLossRate + 1)) / 2;

                console.log("仓位过大，盈利取保底价，绘制网格 0/3交易点改变：", curMinPrice, gridPoints);
            }
            if (allPoints >= 8) {
                // 发送邮件
                sendMail({
                    subject: `${B_SYMBOL}仓位过大，交替穿过${allPoints}次交易点，请手动处理`,
                    text:
                        `交替穿过${allPoints}次交易点` +
                        JSON.stringify({
                            gridPoints,
        ...
        // 发送邮件
        sendMail({
            subject: `${B_SYMBOL}不可能到这里，'len !== 2 && tradingSwitch === TradingStatus.Play'，请手动处理`,
            text:
                `交替穿过${allPoints}次交易点` +
                JSON.stringify({
                    gridPoints,
