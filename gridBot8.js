...
let loadingCloseTradeBySide = false; // 平仓（需要side参数）
let loadingReverseTrade = false; // 反手
let loadingForehandTrade = false; // 顺手
let loadingCloseCurTrade = false; // 平仓
let loadingNewPoints = false; // 修改网格
const isLoading = () => {
    return loadingPlaceOrder || loadingCloseTradeBySide || loadingReverseTrade || loadingForehandTrade;
};
...
// 设置网格
const setGridPoints = (_currentPrice) => {
    loadingNewPoints = true;
    // 先清空以前的
    gridPoints = [];
    repeatSamePointMap = {};
    // 保证当前价在网格正中间
    let price = (price2 = _currentPrice + gridHight * 0.5 * 0.99999999); // 0.999999999是为了让小数位数够多
    let num = gridCount;
...
// 平仓（需要side参数）
const closeTradeBySide = async (side) => {
    if (loadingCloseTradeBySide) return;
    loadingCloseTradeBySide = true;
    try {
        const timestamp = Date.now() + serverTimeOffset;
        let params = {
            symbol: B_SYMBOL, // 交易对
        ...
        loadingCloseTradeBySide = false;
    } catch (error) {
        console.error(
            "closeTradeBySide header::",
            error && error.request ? error.request._header : null,
            " error::",
            error && error.response ? error.response.data : error,
        );
...
// 根据方向平仓
const closeCurTrade = async () => {
    if (loadingCloseCurTrade) return;
    loadingCloseCurTrade = true;
    console.log("开始平仓:");
    try {
        // 如果有开多的仓位，执行反手操作
        if (store.side === "BUY") {
            console.log("closeCurTrade ~ 有开多的仓位，执行平仓, currentPrice:", currentPrice);
            // 先平多
            await closeTradeBySide("SELL");
            console.log("平多完成");
        ...
        // 如果有开空的仓位，执行反手操作
            ...
            console.log("closeCurTrade ~ 有开空的仓位，执行平仓, currentPrice:", currentPrice);
            // 先平空
            await closeTradeBySide("BUY");
            console.log("平空完成");
        ...
        // recordPurchaseInfo({ entryGridPoint: curGridPoint });
        loadingCloseCurTrade = false;
    } catch (error) {
        console.error("closeCurTrade error::", error);
        process.exit(1);
    }
};
...
const confirmOrder = async () => {
    ...
    try {
        if (purchaseInfo.trend) {
            ...
            // 上升趋势
            if (trend === "up") {
                console.log("🚀 ~ 当前价格大于上次交易价，走势为：上升，开多");
                await placeOrder("BUY", getQuantity()); // 开多
            }

            // 下降趋势
            if (trend === "down") {
                console.log("🚀 ~ 当前价格小于上次交易价，走势为：下降，开空");
                await placeOrder("SELL", getQuantity()); // 开空
            }
            // 每次到达交易点必须修改 entryGridPoint orderPrice, orderPrice 在下单时已经做了
...
const setInitData = (orderDetail) => {
    const { positionAmt, entryPrice, unRealizedProfit } = orderDetail;
...
const startTrading = async () => {
    try {
        ...
        setGridPoints(currentPrice); // 绘制网格
...
const gridPointTrading = async () => {
    ...
    // 到达不同网格交易点或者还没到过任何交易点，先平仓好一点更快，不需要考虑插针了，稳就是赚
    if (purchaseInfo.entryGridPoint !== curGridPoint) {
        console.log("不同交易点：立马平仓，preGridPoint, curGridPoint::", purchaseInfo.entryGridPoint, curGridPoint);
        // 到达不同交易点，先立马平仓
        await closeCurTrade();
    } else {
        // 避免反手亏太多，重新绘制网格
    ...
    console.log("先不下单，先改purchaseInfo，等三分钟", purchaseInfo);
    // 每次到达交易点必须修改 entryGridPoint orderPrice, orderPrice 在下单时已经做了
    recordPurchaseInfo({ entryGridPoint: curGridPoint, orderPrice: 0 });
...
const startRunGrid = async () => {
    ...
    if (_curGridPoint) {
        ...
        console.log("到达交易点 curGridPoint:", curGridPoint, "立马执行");
    ...
    // await checkOrderBySpace(); // socket 更新频率并不高，需要节流
};
...
const createLogs = () => {
    ...
    // 重定向 console.log 到文件
    logStream = fs.createWriteStream(`${logsFolder}/${getDate()}.log`, { flags: "a" });
    ...
    errorStream = fs.createWriteStream(`${errorsFolder}/${getDate()}.error`, { flags: "a" });
