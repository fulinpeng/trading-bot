...
let historyEntryPointsLen = Number(process.argv[3]);
let point1 = Number(process.argv[4]);
let point2 = Number(process.argv[5]);
...
if (!historyEntryPointsLen) {
    console.error("请提供historyEntryPointsLen");
    process.exit(1);
}
if (!point1) {
...
const setNewData = async () => {
    writeInFile(`./garbage/prod-mading-${S_SYMBOL}.${getDate()}.js`, data);
    const { up, down } = await getPositionRisk(); // 获取当前仓位信息;
    if (up && up.quantity) {
        const orderPrice = up.orderPrice;
        data.historyEntryPoints = new Array(historyEntryPointsLen - 2).fill(-2);
        data.historyEntryPoints.push(1, 2);
    ...
    if (down && down.quantity) {
        data.historyEntryPoints = new Array(historyEntryPointsLen - 2).fill(-2);
        data.historyEntryPoints.push(2, 1);
    ...
    writeInFile(`./data/prod-mading-${S_SYMBOL}.js`, data);
