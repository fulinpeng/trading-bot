...
const dataFilePath = path.resolve(__dirname, "../../tests/source", `${symbol}-1m.js`);
...
const getKLineData = async (symbol, interval, limit, startTime) => {
    try {
        ...
        console.log("🚀 ~ file: getKlines-1m-to-now.js ~ getKLineData ~ error:", error);
        return;
    }
};
...
const getDatas = async (symbol) => {
    ...
    for (let i = 0; i < num; i++) {
        ...
        if (resItem) {
            ...
            console.log("🚀 ~ file: getKlines-1m-to-now.js ~ getDatas ~ resItem:", resItem);
    ...
    const filePath = path.resolve(__dirname, "../../tests/source", `${symbol}-1m.js`);
