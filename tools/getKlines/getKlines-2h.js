...
const getKLineData = async (symbol, interval, limit, startTime) => {
    try {
        ...
        console.log("🚀 ~ file: getKlines-2h.js ~ getKLineData ~ error:", error);
        return;
    }
};
...
const getDatas = async (symbol, startTime, num) => {
    ...
    for (let i = 0; i <= parseInt(num / 4) + 1; i++) {
        ...
        if (resItem) {
            ...
            console.log("🚀 ~ file: getKlines-2h.js ~ getDatas ~ resItem:", resItem);
