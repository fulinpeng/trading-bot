// https://fapi.binance.com/fapi/v1/klines?symbol=zkUSDT&interval=30m&limit=960&startTime=1721836800000
...
const dataFilePath = path.resolve(__dirname, "../../tests/source", `${symbol}-30m.js`);
...
const getKLineData = async (symbol, interval, limit, startTime) => {
    try {
        ...
        console.log("🚀 ~ file: getKlines-30m-to-now.js ~ getKLineData ~ error:", error);
        return;
    }
};
...
const getDatas = async (symbol) => {
    ...
    let limit = 24 * 2 * 20; // 20天有 30m 级别k线 960 根
    let twentyDay = 24 * 20 * 60 * 60 * 1000; // ms
    let num = parseInt((Date.now() - startTime) / twentyDay); // 多少个twentyDay（请求多少次）
    let rest = parseInt(((Date.now() - startTime) % twentyDay) / 1000 / 60 / 30);

    let isErro = false;
    for (let i = 0; i < num; i++) {
        let _startTime = startTime + twentyDay * i;
        let resItem = await getKLineData(symbol, `30m`, limit, _startTime);
        if (resItem) {
            result = result.concat(resItem);
        } else {
            console.log("🚀 ~ file: getKlines-30m-to-now.js ~ getDatas ~ resItem:", resItem);
    ...
    if (rest && !isErro) {
        let _startTime = startTime + twentyDay * num;
        limit = rest;
        let resItem = await getKLineData(symbol, `30m`, limit, _startTime);
        if (resItem) {
            result = result.concat(resItem);
