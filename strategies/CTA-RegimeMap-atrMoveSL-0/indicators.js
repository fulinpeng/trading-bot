...
async function setEveryIndex(historyClosePrices, curKLine, state, config) {
    ...
    const promises = [
        ...
        new Promise(resolve => setImmediate(() => {
            setAtrMoveStopArr(curKLine, state, config);
            resolve();
        })),
    ];
...
/**
 * 计算用于 profitR 移动止损的 ATR
 * - 只取最新 ATR 值并入栈到 atrForMoveSLArr
 * - ATR 周期取 config.moveStopAtrPeriod（默认 14）
 * @param {Array} klines - K线数据数组
 * @param {Object} state - 策略状态对象
 * @param {Object} config - 配置对象
 */
function setAtrMoveStopArr(klines, state, config) {
    const { atrForMoveSLArr } = state;
    const { moveStopAtrPeriod = 14 } = config || {};

    // calculateATR 需要至少 period 个 TR 值，实际这里用 period+1 保证更稳定
    const minRequired = moveStopAtrPeriod + 1;
    if (!klines || klines.length < minRequired) {
        atrForMoveSLArr.length >= minRequired && atrForMoveSLArr.shift();
        atrForMoveSLArr.push(null);
        return;
    }

    const atrSlice = klines.slice(-minRequired);
    const atrResult = calculateATR(atrSlice, moveStopAtrPeriod);
    const atrValue = atrResult?.atr ?? (atrResult?.atrArr && atrResult.atrArr[atrResult.atrArr.length - 1]);

    atrForMoveSLArr.length >= minRequired && atrForMoveSLArr.shift();
    atrForMoveSLArr.push(atrValue && isFinite(atrValue) && atrValue > 0 ? atrValue : null);
}
