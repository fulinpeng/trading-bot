...
const state = {
    ...
    atrForMoveSLArr: [], // 用于 profitR 移动止损的 ATR 数组
...
const refreshKLineAndIndex = async (curKLine) => {
    ...
    // 日志采集移动到本根K线处理结束后，确保止损线等实时状态已更新
};
    ...
    try {
        ...
        if (response && response.data && response.data.origQty) {
            ...
            if (isPartialClose) {
                ...
                console.log("🚀 ~ closeOrder ~ 平仓时的indicatorTPCount:", state.indicatorTPCount, "profitR:", state.profitR);
                // 全部平仓：重置所有状态
                state.readyTradingDirection = "hold";
...
const closeUp = async (quantity, stopPrice) => {
    ...
    await closeOrder("SELL", closeQuantity, () => { // 该callback在平仓后会立即执行，比重置变量要早
        const curTestMoney =
        ...
        const profitR = state.profitR ?? null;
        if (enableVisualizationLogs) {
            const collector = getLogCollector();
            if (collector) {
                collector.recordClose(kLineDate, _currentPrice, state.testMoney, state.tradingInfo.orderPrice, profitR);
            }
        }
    });
};
...
const closeDown = async (quantity, stopPrice) => {
    ...
    // 注意：initialPositionSize 应该在 judgeProfitRunOrProfit 函数开始时记录（与Pine Script一致）
    await closeOrder("BUY", closeQuantity, () => { // 该callback在平仓后会立即执行，比重置变量要早
        const curTestMoney =
...
const startWebSocket = async () => {
    ...
    ws.on("message", async (data) => {
        ...
        if (isNewLine) {
            ...
            // 收集可视化日志数据（每根K线一次，放在交易逻辑之后，确保止损线等实时状态已更新）
            if (enableVisualizationLogs) {
                const collector = getLogCollector();
                if (collector && collector.enabled) {
