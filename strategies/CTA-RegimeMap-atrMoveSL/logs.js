...
class LogCollector {
    constructor(config) {
        ...
        this.data = {
            ...
            stopLossLineHistory: [], // 止损线（移动止损 || 初始止损 || null），与K线一一对应
            orderAmountHistory: [], // 订单金额
            curTestMoneyHistory: [], // 当前测试资金历史
            profitRHistory: [],     // 每次平仓时的 profitR（与 closeHistory 一一对应）
    ...
    collectIndicatorArrays(state) {
        ...
        // 止损线：移动止损优先，其次初始止损；无订单为 null
        let stopLossLine = null;
        if (state && state.hasOrder && state.tradingInfo && state.tradingInfo.trend) {
            const isLong = state.tradingInfo.trend === 'up';
            const initialSL = isLong ? state.initialLongStopLoss : state.initialShortStopLoss;
            const moveSL = state.trailActive && state.trailStop !== null ? state.trailStop : null;
            stopLossLine = (moveSL !== null && moveSL !== undefined) ? moveSL : (initialSL ?? null);
        }
        this.data.stopLossLineHistory.push(stopLossLine);
    }
     ...
     * @param {number|null} profitR - 平仓时的盈利倍数 R（部分平仓可为 null）
     */
    recordClose(time, price, testMoney, orderPrice, profitR) {
        if (!this.enabled) return;
        ...
        this.data.profitRHistory.push(profitR);
    }
    ...
    saveToFile() {
        ...
        var stopLossLineHistory = ${JSON.stringify(this.data.stopLossLineHistory, null, 8)}
        ...
        var profitRHistory = ${JSON.stringify(this.data.profitRHistory, null, 8)}
