...
let orderPriceForCloseHistory = [];
let initialStopLossHistory = [];
let stopLossLineHistory = [];
...
function reloadData() {
    try {
        ...
        if (fs.existsSync(logDataPath)) {
            ...
            orderPriceForCloseHistory = logData.orderPriceForCloseHistory || [];
            initialStopLossHistory = logData.initialStopLossHistory || [];
            stopLossLineHistory = logData.stopLossLineHistory || [];
            ...
            profitRHistory = logData.profitRHistory || [];
            indicators = {
...
app.get("/", (req, res) => {
    ...
    const injectedHTML = htmlTemplate
        ...
        .replace("{{orderPriceForCloseHistory}}", jsonString(orderPriceForCloseHistory))
        .replace("{{initialStopLossHistory}}", jsonString(initialStopLossHistory))
        .replace("{{stopLossLineHistory}}", jsonString(stopLossLineHistory))
        ...
        .replace("{{profitRHistory}}", jsonString(profitRHistory))
