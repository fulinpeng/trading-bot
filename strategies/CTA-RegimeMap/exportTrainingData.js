/**
 * 导出 ML 训练数据
 * 根据开仓时记录的特征 + 完整 K 线计算 label（未来 N 根内 maxProfitR 是否 ≥ trainingLabelThreshold，默认 1R）
...
const DEFAULT_LABEL_THRESHOLD = 0.5; // 与 md：maxProfitR >= 1 则 label=1
 ...
 * @param {number} labelThreshold - 达到多少 R 记为 label=1（如 1 表示 1R）
 * @returns {Array} 带 label 的样本
 */
function computeLabels(trainingSamples, kLineData, futureBarsCount = 100, labelThreshold = DEFAULT_LABEL_THRESHOLD) {
    const result = [];
    for (const sample of trainingSamples) {
        ...
        const label = maxProfitR >= labelThreshold ? 1 : 0;
        result.push({
            ...sample,
            is_long: isLong ? 1 : 0,
 ...
 * @param {number} [options.trainingLabelThreshold] - label=1 当 maxProfitR >= 该值（R），默认 1
...
function exportTrainingData(options) {
    const {
        ...
        klineStage = "30m",
        futureBarsCount = 100,
        trainingLabelThreshold = DEFAULT_LABEL_THRESHOLD,
        outputPath,
    } = options;
    ...
    const samplesWithLabels = computeLabels(
        trainingSamples,
        kLineData,
        futureBarsCount,
        trainingLabelThreshold
    );
    ...
    const output = {
        meta: {
            ...
            klineStage,
            futureBarsCount,
            labelThreshold: trainingLabelThreshold,
