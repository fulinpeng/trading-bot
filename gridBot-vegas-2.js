...
const config = require("./config-vegas-2.js");
...
const initEveryIndex = (prices) => {
    const len = prices.length;
    for (let i = len - 20; i < len; i++) {
        setEmaArr(prices.slice(0, i));
    }
};
...
const setEmaArr = (prices) => {
    emaFast.length >= 10 && emaFast.shift();
    ema144.length >= 10 && ema144.shift();
    ema169.length >= 10 && ema169.shift();

    emaFast.push(calculateEMA(prices, EMA_PERIOD[0], emaFast[emaFast.length - 1]));
    ema144.push(calculateEMA(prices, EMA_PERIOD[1], ema144[ema144.length - 1]));
    ema169.push(calculateEMA(prices, EMA_PERIOD[2], ema169[ema169.length - 1]));
};
...
const refreshKLineAndIndex = (curKLine) => {
    ...
    if (isTest) {
        console.log("🚀 ~ 各种指标: curKLine:", curKLine);
    }
};
...
const judgeTradingDirection = () => {
    const [kLine1, kLine2, kLine3] = getLastKlines(kLineData, 3);
...
const getHistoryData = () => {
    if (fs.existsSync(`./data/${isTest ? "test" : "prod"}-vegas-${SYMBOL}.js`)) {
        let historyDatas = require(`./data/${isTest ? "test" : "prod"}-vegas-${SYMBOL}.js`);
        const {
...
const createLogs = () => {
    ...
    // 重定向 console.log 到文件
    logStream = fs.createWriteStream(`${logsFolder}/${isTest ? "test" : "prod"}-vegas-${SYMBOL}-${getDate()}.log`, {
        flags: "a",
    });
    ...
    errorStream = fs.createWriteStream(
        `${errorsFolder}/${isTest ? "test" : "prod"}-vegas-${SYMBOL}-${getDate()}.error`,
        { flags: "a" },
    );
...
function saveGlobalVariables() {
    setTimeout(() => {
        ...
        if (currentPrice !== 0 && prePrice !== 0) {
            ...
            fs.writeFileSync(`data/${isTest ? "test" : "prod"}-vegas-${SYMBOL}.js`, `module.exports = ${data}`, {
                flag: "w",
            });
