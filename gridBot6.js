...
let availableMoney = 6; // 可用的USDT数量
const invariableBalance = true; // 是否使用固定金额建仓，为true时，availableMoney为必填
let leverage = 1; // 杠杆倍数
const priceScale = 0.15; // 价格缩放区间（0.05表示当前价格上下浮动5%）
let gridCount = 100; // 多少个格子
let gridHight = 0; // 格子高度
const needSellPercent = 4; // 为 3 表示买反了并超过三分之一个格子就需要反手
// 0.0011 => Math.abs(103.026-103.025)
// 0.00011 => Math.abs(1.4412-1.4412)
// 0.000011 => Math.abs(0.11894-0.11894)
// const PERCENTAGE_THRESHOLD = 0.000011; // Math.abs(103.026-103.025) <= 0.0011 ==> 可以匹配（103.024～103.026），多一个1是因为有的数据加起来是无限小数
let currentPrice = 0; // 记录当前价格
let receivePointPrices = [0, 0];
...
const isLoading = () => {
    return loadingPlaceOrder || loadingCloseOrder || loadingReverseTrade;
};
...
const getStoreInfo = async (orderId) => {
    try {
        ...
        console.warn("getStoreInfo error:", error);
    }
};
// 获取持仓风险
...
const entryGridPoint = async () => {
    console.log("价格到网格交易点::isLoading，curGridPoint, ", isLoading(), curGridPoint);
    if (isLoading()) return;

    // 到达不同网格交易点或者还没到过任何交易点，先不平仓好一点更快市场瞬息万变，需要快
    if (purchaseInfo.entryGridPoint !== curGridPoint) {
        const [preP, curP] = receivePointPrices;
        // >>>>>>
        // sum +=
        ...
        // 在调用此函数时，就避免 preP 和 curP 相等？？？？？？不能相等
        // 处理跨多个交易点时订单价格未更新
        // 每次到达交易点必须修改 entryGridPoint orderPrice
        // 该函数中 curGridPoint 肯定是存在的，因为到了交易点该函数才会触发
...
// 立即执行，如果需要反手就反手，不需要就保持不变，第二次出交易点再执行也不亏需要反手就反手，不需要就保持不变，这个策略挺好的(一分钟，离开时才触发)
const realEntryGridPoint = debounce(entryGridPoint, 60000, false);
// 价格空间上保证订单少亏损（买错了要反手）
...
const startRunGrid = async () => {
    if (prePrice === currentPrice) return; // 相等的话直接退出，因为它到不了任何交易点，继续执行也没有意义
    ...
    if (_curGridPoint) {
        ...
        await realEntryGridPoint(); // 防抖，避免频繁反手，反正1分钟内进来多少次，都是出了交易点才买
    }
...
const startWebSocket = async () => {
    ...
    ws.on("message", async (data) => {
        ...
        receivePointPrices = [prePrice, currentPrice];
        // 测试插针 >>>>>>>
        // if (sum <= -10) {
        //     console.log("爆仓～");
        // }
