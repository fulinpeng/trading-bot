...
const setInitData = ({ up, down }) => {
    ...
    if (fs.existsSync(`data/${SYMBOL}.js`)) {
        ...
        if (
            ...
            ((up
                ? __tradingDatas[__currentPointIndex].up &&
                  up.quantity == __tradingDatas[__currentPointIndex].up.quantity
                : false) ||
                (down
                    ? __tradingDatas[__currentPointIndex].down &&
                      down.quantity == __tradingDatas[__currentPointIndex].down.quantity
                    : false))
        ) {
            historyEntryPoints = __historyEntryPoints;
            ...
            console.log("该币现有仓位和上次保留的数据不符合，请先手动平仓！！！");
            process.exit(1);
        }
        console.log(
...
const gridPointTrading2 = async () => {
    ...
    if (len === 1) {
        ...
        // if (allPoints === 4) {
        //     if (_currentPointIndex === 1 || _currentPointIndex === 2) {
        //         const { trend } = await calcEma1Ema2({ emaPeriod1: 6, emaPeriod2: 13, threshold: 0 });

        //         // 反着开仓
        //         if (_currentPointIndex === 1 && trend == "up") {
        //             console.log(
        //                 `交替穿过${allPoints}次交易点，赌一把反向仓位,重新设置止盈区间（盈利1 或 血亏0.6*4），当前index为1，trend为${trend}`,
        //                 gridPoints,
        //             );
        //             await closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex);
        ...
        //             return; // 退出
        //         }
        //         if (_currentPointIndex === 2 && trend == "down") {
        //             console.log(
        //                 `交替穿过${allPoints}次交易点，赌一把反向仓位,重新设置止盈区间（盈利1 或 血亏0.6*4），当前index为2，trend为${trend}`,
        //                 gridPoints,
        //             );
        //             await closeOtherPointAllOrders(pointIndexHistory, _currentPointIndex);
